...
import { Plus, BarChart3, Settings, Info, AlertTriangle } from 'lucide-react';
...
export function PricingTierManager({ productId, productName, productType }: PricingTierManagerProps) {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(`/api/products/${productId}/pricing-tiers/stats`);
      if (response.ok) {
        const data = await response.json();
        setStats(data.stats);
      }
    } catch (_error) {
      // console.error('Error fetching pricing tier stats:', error);
    }
  };
  ...
  const handleSubmitTier = async (data: any) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || `Failed to ${editingTier ? 'update' : 'create'} pricing tier`);
      }
      ...
      toast.error(error instanceof Error ? error.message : `Failed to ${editingTier ? 'update' : 'create'} pricing tier`);
    } finally {
  ...
  const handleDeleteTier = async (tier: PricingTier) => {
    try {
      const response = await fetch(`/api/products/${productId}/pricing-tiers/${tier.id}`, {
        method: 'DELETE',
        headers: createAPIHeaders(),
      });
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to delete pricing tier');
    }
  };
  ...
  const handleToggleActive = async (tier: PricingTier) => {
    try {
      ...
      toast.success(`Tier ${!tier.isActive ? 'activated' : 'deactivated'} successfully`);
      fetchTiers();
    } catch (error: unknown) {
      // console.error('Error toggling tier status:', error);
      toast.error(error instanceof Error ? error.message : 'Failed to update tier status');
    }
  };
  ...
  const handleReorderTiers = async (newOrder: PricingTier[]) => {
    try {
      ...
      const response = await fetch(`/api/products/${productId}/pricing-tiers/reorder`, {
        method: 'POST',
        headers: createAPIHeaders(),
      ...
      toast.error(error instanceof Error ? error.message : 'Failed to reorder tiers');
    }
  };
    ...
    <div className="space-y-6">
            ...
            <CardContent className="p-4">
              <div>
                ...
                <p className="text-lg font-bold">
                  {formatPrice(stats.priceRange.min, 'usd')} - {formatPrice(stats.priceRange.max, 'usd')}
                </p>
              </div>
            </CardContent>
          </Card>
      ...
      {tiers.length > 0 && activeTiers.length === 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            All pricing tiers are inactive. Customers won't be able to purchase this product.
          </AlertDescription>
        </Alert>
      )}

      {tiers.length > 1 && !hasFreemiumTier && productType === 'SUBSCRIPTION' && (
        <Alert>
          <Info className="h-4 w-4" />
          <AlertDescription>
            Consider adding a freemium tier to attract more customers to your subscription product.
          </AlertDescription>
        </Alert>
      )}
