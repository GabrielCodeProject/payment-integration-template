...
const CSRF_TOKEN_NAME =
  process.env.NODE_ENV === "production" ? "__Secure-csrf-token" : "csrf-token";
...
export function validateCSRFProtection(
  ...
  if (request.method === "POST") {
    ...
    if (
      nextAction ||
      nextRouter ||
      contentType.includes("text/plain;action=")
    ) {
      return { isValid: true };
    }
  }
  ...
  if (isDevelopment) {
    console.log("🔒 CSRF Validation Debug:", {
      ...
      cookieToken: cookieToken
        ? `${cookieToken.substring(0, 8)}...`
        : "missing",
      headerToken: headerToken
        ? `${headerToken.substring(0, 8)}...`
        : "missing",
...
export const sensitiveCSRFConfig: CSRFConfig = {
  ...
  excludePaths: ["/api/stripe/webhook", "/api/health"], // Remove auth routes from exclusions for stricter protection
};
